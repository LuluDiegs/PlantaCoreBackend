...
O arquivo deve ser criado no caminho relativo `./PlantaCoreAPI.API/.env`.  
Use o arquivo `./PlantaCoreAPI.API/.env.example` como referência para estruturação do arquivo.
...
> [!WARNING]  
> Os valores do arquivo `.env` são mantidos fora do versionamento no Git por conterem informações sensíveis.  
...
O arquivo deve ser criado no caminho relativo `./PlantaCoreAPI.IntegrationTests/.env`.  
Use o arquivo `./PlantaCoreAPI.IntegrationTests/.env.example` como referência para estruturação do arquivo.
